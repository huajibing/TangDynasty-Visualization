...
export function createHouseholdSizeScale(
  domain,
  range = DEFAULT_RANGE,
  { nice = true } = {},
) {
...
export function createProductRichnessScale(
  domain,
  range = DEFAULT_RANGE,
  { nice = true } = {},
) {
...
export function createPopulationRadiusScale(
  domain,
...
export function createProductTypeColorScale(domain = PRODUCT_TYPE_KEYS) {
  const d3Ref = ensureD3();
  const safeDomain =
    Array.isArray(domain) && domain.length > 0 ? domain : PRODUCT_TYPE_KEYS;
  const colors = safeDomain.map(
    (key) => COLORS.productTypes[key] || COLORS.theme.secondary,
  );
...
export function createDaoColorScale(domain = Object.keys(COLORS.daos)) {
  ...
  const safeDomain =
    Array.isArray(domain) && domain.length > 0
      ? domain
      : Object.keys(COLORS.daos);
  const colors = safeDomain.map(
    (key) => COLORS.daos[key] || COLORS.theme.primary,
  );
  return d3Ref.scaleOrdinal().domain(safeDomain).range(colors);
}
