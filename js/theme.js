...
export function initTheme(options = {}) {
  ...
  const initialMode =
    stored || (options.defaultMode === 'light' || options.defaultMode === 'dark'
      ? options.defaultMode
      : 'auto');
...
export function cycleThemeMode() {
  const next =
    currentMode === 'auto' ? 'light' : currentMode === 'light' ? 'dark' : /** @type {ThemeMode} */ ('auto');
  applyTheme(next);
...
export function bindThemeToggle(target) {
  ...
  const render = (state) => {
    ...
    if (iconEl) {
      // ☀ 日间 / ☾ 夜间 / A 自动
      iconEl.textContent = mode === 'auto' ? 'A' : theme === 'dark' ? '☾' : '☀';
    }
    if (labelEl) {
      labelEl.textContent = mode === 'auto' ? '自动' : theme === 'dark' ? '夜间' : '日间';
    }
