...
class ProductRanking extends BaseChart {
  ...
  _renderRows() {
    ...
    rows
      ...
      .on("mouseenter", (event, d) => {
        ...
        eventBus.emit(EVENTS.PRODUCT_HOVER, {
          ids: d.locationIds,
          product: d.product,
        });
      })
      ...
      .on("click", (_event, d) => {
        const nextSelection =
          this.currentSelection === d.product ? null : d.product;
  ...
  highlightByIds(ids = []) {
    ...
    this.rows
      .classed("is-highlighted", (item) =>
        item.locationIds.some((id) => idSet.has(id)),
      )
      .classed("is-dimmed", (item) => {
        if (!hasIds && !this.currentSelection) return false;
        const matched = item.locationIds.some((id) => idSet.has(id));
        const selected =
          this.currentSelection && item.product === this.currentSelection;
